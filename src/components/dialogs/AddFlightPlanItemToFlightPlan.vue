<script setup>
        ...
        try {
            await Promise.all(
                selectedFlightPlanItems.value.map((item) => {
                    flightPlanItemServices.createFlightPlanItem(item);
                })
            );
    ...
    const handleCardClick = (item) => {
        if (selectedFlightPlanItems.value.includes(item)) {
            selectedFlightPlanItems.value =
                selectedFlightPlanItems.value.filter((selectedItem) => {
    ...
    const fetchItems = async () => {
        const [taskResponse, experienceResponse, flightPlanResponse] =
            await Promise.all([
    ...
    <v-dialog v-model="visible">
        ...
        <v-card v-else class="rounded-lg bg-background">
            ...
            <v-fade-transition mode="out-in">
                ...
                <div v-else>
                    <p class="text-center">Selected Flight Plan Items</p>
                    <v-list class="bg-background scrollable-list">
                        <v-list-item
                            v-for="item in selectedFlightPlanItems"
                            :key="item.id"
